...
| `tests/test_seed_idempotent.py` | 17 (as of 2026-09-11) | `seed_defaults.seed()` run twice is a no-op; the S2.2 (`crunchyrollcdn.com` mode) fix; confirms `crunchyrollsvc.com` (dev-only, removed) is never seeded; regression guards for the six 2026-09-11 v2fly-rebase decisions (AI rebased, five others deliberately not) |
...
| `tests/test_dashboard.py` | 528 (this table's per-file counts are largely unmaintained -- treat as approximate, `pytest --collect-only` is authoritative) | Flask test client: admin auth, user/domain CRUD (incl. `add_show`/`remove_show`, `user_detail`, `domain_detail`, `update_domain`), all of Settings (incl. changing the admin login itself), report-approve (site and show), CSRF/cross-origin guard, bulk schedule access assignment |
...
| `tests/test_squid_conf_regressions.py` | 9 | Config-text/structure regressions in `proxy/squid.conf.template` that a pure-logic unit test can't catch (see below), plus two 2026-08-30 regression guards keeping the deliberately-deferred `ssl_bump`/`http_access` catch-all flip (see `docs/security/overview.md` §3, RoadMap.md) from being silently re-flipped, plus 2026-09-11's guard that `reply_header_access Alt-Svc deny bumped` (finding #2's QUIC-fallback-UX fix) stays present |
