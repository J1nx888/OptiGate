...
//
// baselineRules' `ct mark set 0x1` (bypass_v4/authenticated_v4 only) is
// set once, on a connection's first packet, and persists for that
// connection's entire lifetime (ct mark, not meta mark, specifically so
// DOCKER-USER's forwarding-accept rule keeps seeing it at the FORWARD
// hook long after prerouting -- see that function's own doc comment for
// why). That means reclassifying a device to something MORE restrictive
// (unauthenticated_v4/quarantine_v4) does not by itself revoke an
// already-accepted connection -- only a NEW connection is evaluated
// fresh against the device's current classification. This function
// closes that gap by dropping the device's existing conntrack state so
// its next packet is forced to re-evaluate.
//
// Shells out to conntrack-tools' own `conntrack` CLI -- a different
// kernel subsystem and tool than `nft` (which knftables itself already
// shells out to): nftables' rule engine has no equivalent "delete an
// already-tracked connection" primitive of its own.
func FlushConntrackForSource(ctx context.Context, ip string) error {
	...
	// and this exact stderr text is the only way to tell them apart.
	// "Nothing matched" is the common, expected case (most
