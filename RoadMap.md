...
controller AdGuard-cred bug found + fixed live as `f5705a8`).

**Code fixes that landed live this session** (all committed, pushed,
built, deployed, and safe with zero bump devices):
- `f5705a8` -- controller reads AdGuard creds from the DB (credential
  drift that caused the brute-force lockout).
- `3e381d0` -- `phase3/nftables-manager`: drop QUIC (udp/443) for
  `bump_v4` devices, so HTTPS can't escape interception via HTTP-3.
- `89cf895` -- `phase3/nftables-manager`: auto-prune the pre-rename
  `inet parental_proxy` table on startup (it was live at the same
  prerouting hook, shadowing policy -- this **fixes finding #4**).

The rest are **real defects/gaps surfaced by hands-on testing** on
...
1. **~~Crunchyroll per-series whitelist is not enforcing~~ -- WORKS as of
   ...
   *(Historical detail from the 2026-09-10 trace kept below for context.)*
   - **(a) The bump fails on the device.** After a browser restart,
     ...
     every `CONNECT 104.18.34.202:443` -> `NONE_NONE/000`. **Zero**
     Crunchyroll HTTP requests were decrypted in a 4-minute capture;
     `authz_helper` logged **0** crunchyroll decisions. Yet Black Clover
     played -- so playback reaches CR over a path OptiGate never sees.
     Likely: Chrome on Android 7+ ignores user-store CA certs for its
     own connections. An earlier retest DID briefly bump `www.crunchyroll.com`
     OK, so trust state is inconsistent -- needs checking how/where the
     OptiGate CA is installed on the tablet.
   - **(b) `host_verify_strict` (Squid default `on`) breaks spliced
     multi-IP CDN traffic** -- see finding #6, now confirmed a hard
     blocker for playback (the video CDN `vod-*.crunchyrollcdn.com`
     itself gets `NONE_NONE/409` storms), not just noise.
   - **(c) Full-bump is too blunt** -- see finding #7.
   - **(d) The classifier is stale** (the original finding): `common/cr_urls.py`'s
     `classify()` regexes (`/series/<id>`, `/watch/<id>`,
     `/playback/vN/...`, `/content/vN/cms/objects/<id>`) don't match
     current CR web shapes; the seeded `domain_paths` for domain 27 has
     blanket `^/content/v[0-9]+/` and `^/playback/v[0-9]+/` rules that
     allow the entire modern API with no series check. Observed current
     shapes (in `ANALYSIS.md`): `/content/v2/discover/up_next/<SERIES_ID>`
     (**series id in the path**), `/content/v2/cms/objects/<ids>`
     (ids in path; `series` objects carry `series_metadata` + their own
     id, episodes carry `episode_metadata.series_id`). Redesign plan is
...
4. **~~`optigate.home` from a known non-bypass device shows only a login
   ...
   `inet parental_proxy` nftables table: `.30` was still a member of its
   abandoned `unauthenticated_v4` set, whose `tcp dport 80 redirect to
...
5. **~~Report page "Recent activity" -- couldn't see the
   allowed/blocked Result column~~ -- FIXED 2026-09-10 (`6da84c8` +
   `b49fee2` + the truncation change below).** The `Result` column
   (with the `allowed`/`blocked` badge) has been in `REPORT_BODY` at
   **every commit** and prod serves it byte-perfect. The real cause,
   confirmed by the owner once he spotted the scrollbar: the table
   **overflows horizontally**. `REPORT_BODY`'s "Show / Path" cell
   (`row.series_name or row.series_id or row.path`) and the "Domain"
   cell were rendered untruncated -- a `path` is a full URL with a
   query string (a Crunchyroll `/playback/...` URL is 200+ chars) and a
   domain has no spaces to wrap on, so a single long row stretched the
   `<table>` well past its `.table-scroll` container and pushed the
   `Result` column off the right edge. The owner didn't notice the
   horizontal scrollbar and read it as "the column is missing / this is
   a regression".
   - **Layout fix (this change):** new `.cell-truncate` /
     `.cell-truncate-sm` utilities in `app.css` (`max-width` in `ch` +
     `text-overflow: ellipsis`); `REPORT_BODY` wraps the Show/Path
     value in `<span class="cell-truncate" title="…">` and the Domain
     in `<code class="cell-truncate cell-truncate-sm" title="…">`. Full
     value stays in the `title` tooltip and in the DOM text (so
     select/copy is unaffected). Timestamp cell stacked onto two lines
     (`ts[:10]` / `ts[11:19]`) to reclaim width. Result: the `<table>`
     is now width-bounded regardless of row content (~740px vs
     previously unbounded) and fits without a horizontal scrollbar at
     any normal desktop width; `.table-scroll` stays as the safety net
     for a very narrow window.
   - `6da84c8` / `b49fee2` (kept as defensive hardening, not the fix):
     `@app.after_request` sends `Cache-Control: no-store,
     must-revalidate` + `Pragma: no-cache` on every `text/html`
     response, and `sw.js` `CACHE` was bumped `pp-static-v2` ->
     `pp-static-v3` (forces every browser off any older `sw.js`
     revision; the current worker only ever `respondWith`s for
     `/static/`, never a navigation). These make a stale cached page
     impossible going forward but were not what hid the column.
...
7. **Full-bump on a device is too blunt -- needs selective (per-domain)
   ...
   **destination-scoped interception**: redirect only the
   Crunchyroll-family (and Asurascans/Webtoons) domains' traffic to
   Squid, leave the rest completely untouched. This is a new
   `phase3/nftables-manager` capability (a per-domain bump list resolved
   to destination IP ranges, or an `nftables` `socket`/`tproxy` +
   SNI-aware pre-filter), not a config tweak -- and it makes the
   QUIC-drop safe to apply narrowly too. Until it exists, a device that
   needs the Crunchyroll show-whitelist has to accept degraded general
...
## AdGuard drops both rules when two custom rules share one regex body -- confirmed live, fixed

First real interception-mode test against the actual household LAN (previously only ever exercised against disposable test-VM/bridge harnesses or synthetic against-the-real-AdGuard-instance scripts, never real household devices) surfaced a serious, confirmed-live gap: `192.168.1.41` ("Matthew School Laptop", `bump_enabled=0`) reached `crunchyroll.com`/`asurascans.com`/`webtoons.com` (all `mode='bump'` domains it should have been AdGuard-side hard-denied from) and was not subject to the "School" schedule's Entertainment-category block on `netflix.com` either -- "all category blocks are not working for this device," in the owner's own words.

**Root cause #1, confirmed by direct experiment against the real production AdGuard instance (not theorized):** `crunchyroll.com` had TWO separate custom rules live in AdGuard at the same time -- one from `build_rules()` (the bump-mode-domain deny, 23 client IPs) and one from `build_category_deny_rules()` (crunchyroll.com is also present in a subscribed Entertainment-category domain list, 8 client IPs). Identical regex body, different `$client=` lists. AdGuard's own live query log showed `NotFilteredNotFound` for every single query from both `.41` and `.57` across a 15+ minute window -- not a transient recompile-lag artifact, a sustained miss. Reproduced synthetically against the real AdGuard instance (a temporary test rule pair, always restored byte-for-byte afterward and diff-verified): two rules sharing one regex body, different but overlapping client lists, caused AdGuard to return SERVFAIL or NXDOMAIN depending on rule order -- **neither rule fired**, even for a client correctly listed in both.

**Fixed**: `controller/adguard_sync.py` gained `_merge_duplicate_domain_rules()`, run over the fully-assembled managed-rules list in `sync_once()` before it's ever pushed to AdGuard. Groups rules by `(regex body, trailing action)` -- an unscoped rule wins outright over any scoped duplicate for the same action (it already covers every client); otherwise every group's `$client=` lists are unioned into one rule. Deliberately keyed on `(body, suffix)`, not body alone, so a deny rule (`dnsrewrite=...`) is never merged with an ECH-strip rule (`dnstype=HTTPS`) for the same domain -- those are a deliberate, different-purpose pair per `build_ech_strip_rules()`'s own docstring. 8 new tests (`_merge_duplicate_domain_rules` unit tests plus one `sync_once()` integration test reproducing the exact crunchyroll.com dual-source scenario end to end), full suite passing.

**Root cause #2, ALSO confirmed by direct experiment, NOT yet fixed -- bigger and more systemic than root cause #1:** `netflix.com` has only ONE rule (checked directly, no duplicate), correctly scoped to include `.41`, yet still failed to block it. Tested the real singular rule directly (temporarily added the box's own IP to its live `$client=` list, queried as that IP): got netflix.com's real answer, not the rewrite. Control test: pushed an identical-shaped rule for a real, resolvable domain that had never been queried by anything this session (`community.cloudflare.com`) -- that one blocked correctly on the very first query. Conclusion: **AdGuard's DNS answer cache is shared across all clients and is consulted before/instead of re-evaluating `$client=`-scoped custom rules** -- once ANY client resolves a domain to a real answer, every other client (blocked or not) rides that same cached answer until its TTL expires, and if any unrestricted device keeps re-querying the domain, the cache effectively never goes cold. Checked the live AdGuard config: `cache_enabled: true`, `cache_ttl_min: 0`, `cache_ttl_max: 0` (both zero means AdGuard applies no override at all and just respects whatever TTL the upstream returns -- tens of seconds to 300+ for the real domains this project needs to deny). This is a materially bigger problem than the 2026-09-11 "~3.9s recompile-lag" finding (which still applies on top of this) -- it calls into question whether ANY `$client=`-scoped AdGuard rule this project relies on (bump-domain denies, category denies, splice-domain allowlisting) can be trusted to hold once a domain has ever been resolved by an unrestricted device. **Not a code fix in this repo** -- the candidate mitigation is an AdGuard-side config change (set `cache_ttl_max` to a small value, e.g. a few seconds, to bound the staleness window regardless of upstream TTL, or disable `cache_enabled` outright and accept the latency cost of every lookup going upstream) -- left for the owner's explicit decision given the performance/correctness tradeoff, same as the still-open TTL-shortening decision from 2026-09-11. Tracked as a follow-up, not yet actioned.

Both root causes were found using the same non-disruptive methodology as the 2026-09-11 recompile-latency measurement: synthetic rules pushed to the real production AdGuard instance, always restored byte-for-byte and diff-verified afterward, no real device or real rule touched during testing.

**Root cause #2 resolved by owner decision**: rather than shorten `cache_ttl_max` and leave a residual window, the owner opted to close the gap outright -- "the client will cache the responses regardless [on its own side] and we can't let one device that has permissions to a domain affect another device." Disabled AdGuard's DNS cache entirely on the live production instance via `POST /control/dns_config` (`cache_enabled: false`, everything else in the DNS config left untouched), confirmed persisted to `AdGuardHome.yaml` on disk (survives a container restart) and confirmed normal resolution still works (`google.com`/`example.com` both resolve correctly with caching off). No code change needed for this half -- it's an AdGuard-side setting, not this project's own rule-generation logic.

## Domain-pattern input had no defense against a pasted URL -- the actual source of the `w13.returnersmagicshould.com` bug above

Also found while reading the live rules during the investigation above: the `w13.returnersmagicshould.com` domain's stored pattern was literally `https://w13.returnersmagicshould.com` -- a DNS query name never contains a URL scheme, so this rule could never have matched anything regardless of the two AdGuard bugs above. Owner's reaction: "I thought we had checks in place to make sure that any domains pasted were in a format that could be understood or would be rejected" -- correct instinct, but that check only existed on ONE of the input paths.

**Root cause**: `dashboard.py` has two different families of domain-pattern input. `add_domain_from_url()`/`_extract_domain()`/`_extract_path()` (the "paste a URL" shortcuts) already parse the input with `urlparse()` and extract just the hostname/path, so a pasted URL was always safe there. But the plain hand-typed-pattern forms -- `add_domain()` (the main Domains-page "Add domain" form), `add_category_domain()`, and `add_category_override()` -- only checked that the pattern was non-empty, under 200 characters, and a *syntactically valid regex*. `https://w13.returnersmagicshould.com` passes all three checks fine (`:`, `/`, and letters are all valid, harmless regex characters) even though, once anchored by `common/matching.py`'s `_domain_regex()` the same way every stored pattern is, it can never match a real hostname. `add_category_override()` didn't even have the regex-validity check at all.

**Fixed**: new `_looks_like_a_pasted_url()` helper in `dashboard.py` -- rejects any hand-typed pattern containing a URL scheme (`://`), a path separator (`/`), or whitespace, none of which a legitimate domain-matching regex ever needs. Wired into all three hand-typed-pattern routes (`add_domain()`, `add_category_domain()`, and `add_category_override()`, which also gained the missing regex-validity check it never had). Deliberately narrow -- rejects the specific "pasted a URL" mistake without restricting genuine custom-regex use (alternation, character classes, etc. still work fine), and doesn't touch the already-safe URL-extraction paths at all. 5 new tests across `tests/test_dashboard.py` (URL and path-bearing patterns rejected on all three routes, an invalid-regex override now correctly rejected too). Owner manually corrected the one bad live row (stripped the `https://` prefix) before this fix landed.

## Two UI requests from the 2026-09-14 testing session

**Crunchyroll integration page reordered.** Owner's feedback: the "Required domains" (protected-infrastructure) card sat too high on the page -- Approved Shows and the add-a-show flow are what an admin actually comes to this page to do, day to day; the domains card is reference material an admin rarely needs to touch. `INTEGRATIONS_BODY` reordered so Approved Shows and the approve-a-show form come first, Required domains moved to the bottom. Pure template reorder, no route/query/logic changes -- nothing to break.

**Device MAC addresses are now direct links, not a separate "View on Devices" button.** Owner's feedback: `user_detail()`/`group_detail()`'s own "Devices" cards (added 2026-09-08, see this file's own dated entry "Devices reachable directly from a user's/group's own page") linked to the Devices page's `?q=` search instead of the device's own page -- an extra click and a filtered list instead of landing directly on the one device. Report page (`REPORT_BODY`) already had the right pattern (`device_detail`, linking the device's own label/MAC directly), so both cards were changed to match: the MAC address `<code>` itself is now the link (`url_for('device_detail', device_id=d.id)`), and the separate "View on Devices" column is gone. `user_detail()`'s `assigned_devices` query gained the `id` column it didn't select before (`group_detail()`'s `group_devices` already had it). 2 existing tests updated to assert the new direct link and the absence of the old `?q=` one, rather than removed -- the underlying "get from here to the device" capability is unchanged, just how you get there. Full `test_dashboard.py` suite passing.

## AdGuard native-block visibility + dashboard-driven override (root cause #8 above, done properly)

Full implementation of the design discussed above (item 8's "AdGuard blocking things the admin doesn't want blocked, with no visibility or way to fix it" concern) -- planned via `EnterPlanMode` with the owner (two rounds of `AskUserQuestion` to pin down override scope and visibility scope before writing any code), then implemented in full. See the approved plan for the full design rationale; this entry covers what actually shipped.

**Visibility (`dashboard/adguard_report_sync.py`)**: `correlate_once()` now reads AdGuard's own querylog `reason`/`filterId` fields (confirmed via a live pull: real entries carry `{"filterId": 1, "reason": "FilteredBlackList", "rule": "||x^"}`, a shape this module never looked at before). A block whose `filterId` resolves (via `get_filters_status()`) to a URL matching one of the household's own `categories.subscription_url` rows gets `reason="dns_category_deny"` (always shown on the Report page, same treatment `dns_hard_deny` already gets); one that resolves to AdGuard's own built-in filter or a uBlockOrigin extra gets `reason="dns_native_filter_deny"` (hidden behind a new, separate `show_native_blocks` toggle by default -- `show_routine` stays untouched, that one governs allowed-traffic sampling, not blocks). `block_page_ip` is now optional in this whole flow (`str | None`) -- the native-block half needs no block-page IP at all, so native visibility works even when `DASHBOARD_URL` isn't configured; `access_log` gained a `block_source` column (the resolved list/category name) and `logging_util.log_access()` a matching optional parameter.

**Override (`controller/adguard_sync.py` + new `adguard_allowlist` table family)**: new `adguard_allowlist`/`user_adguard_allowlist`/`group_adguard_allowlist`/`device_adguard_allowlist` tables, mirroring `domains`/`user_domains`/`group_domains`/`device_domains` exactly (same shape, same scoping mechanic the owner explicitly asked to reuse) but deliberately a separate table -- a `domains` row is a content-permission decision, an `adguard_allowlist` row is a narrower "never let ANY AdGuard-side list block this, for this audience" claim. `common/matching.py` gained `device_allowlist_reason()`, mirroring `device_domain_reason()`'s exact is_global/user/group/device precedence. `build_adguard_allow_rules()` turns each entry into an AdGuard `@@` (allowlist) rule via new `_domain_allow_rule()` -- AdGuard evaluates `@@` with priority over any blocklist match regardless of source, which is what makes a dashboard-approved exception hold against AdGuard's own built-in filter too, not just this project's own rules. This is the first `@@` rule this project has ever generated (previously an explicit non-goal, see this file's own much older dated note) -- required fixing `_merge_duplicate_domain_rules()`'s `_split_rule()`/`_rebuild_rule()` to be `@@`-prefix-aware (an allow rule and a deny rule for the same domain must never merge into each other, or the merge would erase the exception).

**Report page** (`dashboard.py`): new reason labels; `approve_from_report()` gained a branch for both new reason codes that writes to `adguard_allowlist` (+ the matching scope-table row) instead of falling into the generic tail that creates a `domains` row -- granting a `domains` row would do nothing here, since neither an over-threshold category subscription nor AdGuard's own built-in filter ever consults it. New "AdGuard exceptions" card on the Domains page for direct add/remove (reuses `ACCESS_SELECTS` for the same per-user/group/device scoping UI the Domains page's own add-domain form already uses), plus `_looks_like_a_pasted_url()` validation on the hand-typed-pattern route, same as the earlier domain-input-validation fix.

**Verification**: 39 new tests across `tests/test_controller_adguard_sync.py` (`@@`-merge behavior, `build_adguard_allow_rules()` scoping), `tests/test_adguard_report_sync.py` (category-match and native-fallback resolution, works without a block-page IP), `tests/test_dashboard.py` (allowlist CRUD, the new Report toggle, the new approve branch), and a new `tests/test_db_adguard_allowlist_migration.py` (fresh-schema presence, `block_source` migration without data loss, migration idempotency, cascade-delete behavior across all three scope tables). Full suite: 1485 passed, 35 skipped, zero regressions -- caught and fixed one self-inflicted issue along the way: the new "AdGuard exceptions" card's own hint text used the word "otherwise", which (same class of bug as the `dashboard.py:638` incident from the domain-protection feature) contains "other" as a substring and tripped `test_domain_access_grants_and_filters_by_device`'s `b"other" not in resp.data` assertion; reworded, confirmed fixed, no logic touched. Additionally live-verified end-to-end against the real production AdGuard instance (same non-disruptive push/verify/restore methodology used throughout this session): pushed a synthetic `@@` rule for `ecsv2.roblox.com` (confirmed blocked by AdGuard's own built-in filter, `filterId=1`) scoped to a test client -- that client's query flipped from the `0.0.0.0` block answer to a real, resolved CNAME chain, while a client outside the rule's scope stayed blocked, and the live config was restored byte-for-byte afterward. The `@@` mechanism is proven working against the real box, not just synthetic unit coverage.

**Deployed to production same day**: committed as `69c8b7c` (bundled with the earlier duplicate-rule-merge and domain-validation fixes, plus the two UI fixes from this same session), pushed, pulled on the Beelink (fast-forward from `7df00fc` through the previously-unpushed comment-cleanup commit `583b3a7`). `dashboard`/`proxy`/`controller` all rebuilt (`controller` built but NOT started -- interception profile stays down until the owner brings it up deliberately); `dashboard`/`proxy` recreated. Verified live: both came up with no errors, `/health` still correctly returns 401 without credentials, and the new schema landed cleanly -- `adguard_allowlist` + its three scope tables present, `access_log.block_source` column present, all 3,491 pre-existing `access_log` rows preserved untouched by the migration.

## Gap found answering "does a fresh install get all these fixes too": AdGuard's cache-disable was a live-only change, never codified

Owner's question surfaced a real miss: the cache-disable fix above (root cause #2 of the AdGuard visibility work) was applied ONLY as a live `POST /control/dns_config` call directly against the running production AdGuard instance -- nothing in the codebase actually sets it. A brand-new install, or this same production box if its `adguard` container's config volume were ever recreated, would silently get AdGuard's own default (`cache_enabled: true`) back, reopening the exact shared-cache bug that was just fixed.

Checked the NIC-tuning fix from a prior session for the same class of gap first -- that one IS properly codified (`nic-tuning/optigate-nic-tuning.sh` + its systemd unit, installed automatically by `setup.sh` on Linux-as-root, with manual instructions printed otherwise) -- confirmed by reading `setup.sh` directly, not assumed. The GRO/GSO/TSO offload change is a separate, already-tracked, deliberately-undecided item (not a silent gap) -- left alone.

**Fixed**: `adguard/entrypoint.sh` now forces `cache_enabled: false` on every single boot, both paths:
- **Fresh install**: one more `wget` POST to `/control/dns_config` in the first-boot bootstrap flow, alongside the existing filter-list/update-interval setup calls, using the same admin credentials that flow already has. `AUTH_B64` moved out of the (skippable) extra-blocklists block so it's available unconditionally.
- **Existing/upgrading install**: new `_disable_dns_cache_via_file()`, a `sed -i` edit of `AdGuardHome.yaml` directly, run before AdGuard is launched in the "already configured" branch -- this script has no way to know an existing deployment's live admin password (owned by the dashboard after first boot), so the authenticated API isn't an option there; a direct file edit needs no credentials at all. Idempotent (no-op once already `false`), so it runs unconditionally on every container start, forever -- this production box gets the fix applied automatically the next time its `adguard` container restarts for any reason, with no manual step, and any other existing deployment upgrading onto this code gets the same treatment.

`sh -n` syntax-checked clean. Not yet deployed -- rebuilding/recreating `optigate-adguard` touches the household's live DNS resolver, a more sensitive blast radius than `dashboard`/`proxy`, so this is flagged for the owner's own call on timing rather than deployed in the same breath as everything else.

**Deployed same day, after the owner's own risk correction.** Initial caution above treated `optigate-adguard` as the household's live DNS resolver at all times; the owner corrected this directly -- it's only actually serving DNS to real devices while the `interception` profile is up, and that profile was stopped at the time. Given that, rebuilding/recreating it carried no live blast radius. Deployed in a second pass (separate from the `dashboard`/`proxy`/`controller` deploy above): `optigate-adguard` rebuilt and recreated, came up clean, `cache_enabled: false` confirmed present in the live config with no manual step.

## `$client=` multi-IP AdGuard rules were silently no-ops for every real device -- confirmed live, fixed, deployed

Surfaced immediately on retesting the fixes above with interception back up: `192.168.1.30` ("Matthews Lenovo Tablet", `bump_enabled=0`) could still reach `crunchyroll.com` even though its correctly-scoped deny rule (23 client IPs including `.30`) was live in AdGuard -- this looked at first like a recurrence of the just-fixed duplicate-rule/cache bugs, but both were confirmed still working correctly (cache genuinely disabled, no duplicate rule for this body).

**Root cause, confirmed by direct experiment against the real production AdGuard instance:** every rule builder in `controller/adguard_sync.py` that scopes a rule to more than one device (`_domain_rule()`, `_domain_allow_rule()`, `_ech_strip_rule()`, and `_rebuild_rule()` used by the merge logic) joined multiple client IPs with a comma -- `$client=192.168.1.10,192.168.1.30,...`. AdGuard's actual rule syntax uses comma only to separate *different* modifiers within one `$...` options block (e.g. `$client=X,dnsrewrite=...`); the separator for *multiple values within one modifier* is `|`, the same convention `$domain=a.com|b.com` uses elsewhere in adblock-style filter syntax. A comma-joined multi-IP client list is therefore silently invalid -- and the whole rule becomes a no-op, for every client listed, not just the ones after the first comma.

Proved directly and cleanly: pushed two synthetic rules to the real production AdGuard instance, identical except for delimiter -- `$client=A,B` matched neither `A` nor `B` (NXDOMAIN, rule completely inert); `$client=A|B` matched both correctly (`dnsrewrite` fired, NOERROR with the block-page answer). Cross-checked against real production querylog data: across 2,000 real entries spanning every household device active that day, **zero** ever triggered one of this project's own custom deny/allow rules (`reason=RewriteRule`) -- the only hits at all came from this session's own earlier manual diagnostic scripts, every one of which happened to test with a single stand-in IP (the box's own address), since binding a raw socket to a real device's address from the box itself isn't possible without packet spoofing. A one-element client list has no comma to get wrong, so none of this session's own "confirmed live" verifications (the duplicate-rule fix, the `@@` allow-rule test) ever exercised the multi-IP path at all.

**Impact**: this affected every rule source that can scope to more than one device -- `build_rules()`, `build_splice_deny_rules()`, `build_category_deny_rules()`, `build_ech_strip_rules()`, and `build_adguard_allow_rules()`. In practice, per-device/per-category DNS-tier enforcement has likely never actually held for any domain relevant to more than one denied device at once -- the normal case for nearly every rule in this system. The duplicate-rule and shared-cache fixes earlier in this same session were real and necessary, but this bug meant neither could ever have been confirmed working end-to-end against a real multi-device household until now.

**Fixed**: the four join sites switched to `'|'.join()`. `_split_rule()`'s parser simplified in the same change -- previously it had to comma-split the whole modifier tail and walk forward collecting tokens until it found one containing `=` (since comma was ambiguously both the intra-list AND inter-modifier separator); with `|` as the intra-list separator, a client value can never itself contain a comma, so a single `.partition(",")` on the `client=` tail cleanly separates the client list from everything after it. Existing tests updated to expect `|`-joined output (`test_domain_rule_joins_multiple_client_ips_with_commas` renamed to `_with_pipes`, likewise the ECH-strip equivalent, plus the merge/dedup and `@@` allow-rule tests that asserted multi-IP output). 97 tests in `test_controller_adguard_sync.py` passing, full suite 1485 passed/35 skipped/0 regressions.

**Deployed same day**: committed (`8afe873`), pushed, pulled on the Beelink, `controller` image rebuilt and the container recreated (no other container touched -- this fix lives entirely in `controller/adguard_sync.py`). Came up clean, applied generation 1 with no errors. **Live-confirmed by the owner**: retested Matthew's device immediately after deploy -- crunchyroll.com is now correctly denied.

## Retesting Jacob's Roblox access surfaced a different, unrelated finding: "Mode schedule" doesn't do what the owner expected -- deferred

With the `$client=` fix above deployed, Jacob's Roblox connectivity was retested (the still-open item from 2026-09-14, previously suspected to be AdGuard's own built-in filter blocking `ecsv2.roblox.com`, invisible before this session's AdGuard-visibility work). Fresh querylog immediately after the fix showed the real, current mechanism at work: `roblox.com` and every real subdomain (`apis.roblox.com`, `auth.roblox.com`, `clientsettingscdn.roblox.com`, etc.) getting `reason=RewriteRule` with the block-page answer -- i.e. this project's OWN Games-category rule, now actually enforcing (thanks to the fix above), correctly denying Jacob. Neither of the two prior theories (invisible UDP traffic, invisible native AdGuard block) was the cause; the rule was simply computing the wrong authorization for him.

**Root cause**: Games (category 18, 858 domains, small-scope path via `build_category_deny_rules()`) isn't blocked for Jacob by any base `category_users`/`category_groups`/`category_devices` assignment -- it's only blocked while the "School" schedule (id 5, mon-fri 07:00-15:00 US/Eastern, `is_mode=0`) is active, which it was at test time (a weekday early afternoon). The owner had an active `schedule_overrides` row (id 1, expires same-day 18:49) shifting Jacob into "Free Time (Weekday)" (id 3, `is_mode=1`) specifically to allow this. It had no effect on School, because `common/schedule_eval.py`'s `schedule_is_active_for_target()` only lets an active override suspend a schedule that is ITSELF flagged `is_mode=1` (or `lockout_all=1`) -- a plain schedule like School, with neither flag set, is evaluated purely by the clock, override or not. This is a deliberate, pre-existing design choice (`common/db.py`'s own schema comment on `schedules.is_mode`, and the function's own docstring): "a standing safety-net category block ... must never be silently lifted by someone shifting that kid into Free Time." Worth noting: that same schema comment's example text lists "School / Free Time / Bedtime" together as the canonical example of mutually-exclusive daily modes -- so "School" not carrying `is_mode=1` may simply be an oversight from when these schedules were first set up, rather than a deliberate choice for this specific schedule; not resolved either way this session.

**Why this isn't a quick flag-flip**: "School" is the single schedule gating 16 categories at once via `schedule_categories` -- Adult, Gambling, Drugs, and Fraud & Scams sit on the exact same schedule row as Games, Entertainment, Anime/Comics & Games, YouTube, Social Media, and others. Marking School `is_mode=1` so a Free Time override can lift Games would, by the same mechanism, let that same override lift Adult/Gambling/Drugs/Fraud & Scams for the whole School window too -- a much bigger behavior change than intended, and almost certainly not wanted. A real fix likely means splitting School's category gating into a non-mode "hard" safety-net schedule (Adult/Gambling/Drugs/Fraud & Scams -- never override-able) and a separate mode-eligible schedule for the "soft" school-hours restrictions (Games/Entertainment/etc, override-able the way the owner already expects) -- but this is a product/UX decision about how the "Mode schedule" feature should work in general, not a one-line patch.

**Owner's explicit direction: hold off, no change made.** Flagged for a dedicated future session to discuss "Mode schedule" semantics in depth -- how overrides, hard safety-net schedules, and the mode/non-mode split should actually behave -- rather than patching just this one category split in isolation. Jacob's own test was left to resolve on its own timeline (override expires same-day) rather than worked around live.
